...
export function EditRepairForm({ repair, onSuccess }: { repair: RepairData, onSuccess: () => void }) {
  ...
  const [issue, setIssue] = useState<string>(repair.issue || "");
    ...
    <form action={formAction} className="flex flex-col gap-4 md:gap-6 p-5">
      ...
      <div className="rounded-xl border border-warm-border/50 bg-violet/5 p-4 text-xs text-text-primary space-y-1.5">
        ...
        <div className="space-y-1.5 mt-2">
          <label htmlFor="issue" className="block font-medium text-text-secondary">Несправність</label>
          <textarea
            id="issue"
            name="issue"
            rows={2}
        ...
        {repair.customer_name && <p className="mt-2"><strong className="text-text-secondary">Клієнт:</strong> {repair.customer_name}</p>}
      </div>
