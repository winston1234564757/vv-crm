...
export function RepairsKanban({ repairs, onCardClick, onEditClick }: RepairsKanbanProps) {
  ...
  const renderCard = (repair: RepairRow, col: typeof statusColumns[0]) => {
    const isOverdue =
  ...
  return (
    <div className="space-y-3">
      {error && (
        <div className="rounded-xl bg-rose/10 p-3.5 text-sm text-rose animate-entry border border-rose/20 mx-4 md:mx-0">
          {error}
        </div>
      )}
      ...
      <div className="md:hidden space-y-4">
        ...
        <div className="flex gap-2 overflow-x-auto px-4 pb-2 pt-1 scrollbar-none snap-x snap-mandatory">
          {statusColumns.map((col) => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-1.5 rounded-full px-3.5 py-2 text-xs font-semibold whitespace-nowrap transition-all border snap-center active:scale-95 ${
                  isActive
                    ? `${col.bg} ${col.textColor} border-current shadow-sm`
                    : "bg-warm-sidebar/50 text-text-secondary border-warm-border hover:text-text-primary"
                }`}
              >
        ...
        {(() => {
          const col = statusColumns.find((c) => c.status === activeTab);
          if (!col) return null;
          const colRepairs = repairs.filter((r) => r.status === col.status);
          ...
          return (
            <div className="px-4 pb-4 animate-entry">
              <div className="flex flex-col gap-3">
                {colRepairs.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-12 text-center rounded-2xl border border-dashed border-warm-border bg-warm-sidebar/10">
                    <span className="text-2xl mb-1.5">{col.emoji}</span>
                    <p className="text-xs text-text-muted">Немає ремонтів у статусі "{col.label}"</p>
                  </div>
                ) : (
                  colRepairs.map((repair) => renderCard(repair, col))
                )}
              </div>
            </div>
          );
