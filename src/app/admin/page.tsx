    ...
    <GlassCard interactive>
      ...
      <div className="mt-3 space-y-2">
        {alerts.length === 0 ? (
          ...
          alerts.map((a, i) => (
            <div key={i} className="flex items-center justify-between rounded-xl px-3.5 py-2" style={{ background: a.urgent ? "color-mix(in oklch, var(--color-rose) 8%, transparent)" : "color-mix(in oklch, var(--color-amber) 8%, transparent)" }}>
...
size="default"
      ...
      <div className="grid grid-cols-1 gap-5 md:grid-cols-4">
        <div className="md:col-span-2">
          <GlassCard interactive>
            ...
            <p className="mt-2 text-4xl font-light tracking-tight text-text-primary">{stats.todaySalesTotal.toLocaleString()} грн</p>
        ...
        <GlassCard interactive>
          ...
          {stats.awaitingParts > 0 && (
            <p className="mt-1 text-xs text-amber">{stats.awaitingParts} очікують деталі</p>
          )}
        </GlassCard>
        <GlassCard interactive>
      ...
      <div className="grid grid-cols-1 gap-5 md:grid-cols-4">
        ...
        <GlassCard interactive>
          ...
          <div className="mt-3 space-y-2">
            <AddDeviceButton
              className="flex w-full items-center gap-2 rounded-xl bg-violet/5 px-4 py-3 text-sm font-medium text-violet transition-colors hover:bg-violet/10"
            />
            <AddRepairButton
              customers={customers}
            ...
            {repairs.length === 0 ? (
              ...
              repairs.map((r, i) => (
                <div key={i} className="flex items-center justify-between">
                  <span className="truncate text-sm text-text-primary">{r.device_name}</span>
                  <span className="shrink-0 rounded-lg px-2.5 py-0.5 text-[11px] font-medium" style={{ background: `color-mix(in oklch, ${statusColors[r.status]} 18%, transparent)`, color: statusColors[r.status] }}>
            ...
            {stats.recentSales.map((s, i) => (
              <div key={i} className="flex items-center justify-between py-2.5 first:pt-0 last:pb-0">
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-medium text-text-primary">{s.item}</p>
                  <p className="text-xs text-text-secondary">{s.customer} • {s.time}</p>
                </div>
      ...
      <div className="grid grid-cols-1 gap-5">
        <GlassCard interactive>
          <h2 className="text-sm font-semibold text-text-primary">Активні ремонти</h2>
          <div className="mt-3 space-y-2">
            {repairs.length === 0 ? (
              <p className="text-sm text-text-secondary">Немає активних ремонтів</p>
            ) : (
              repairs.map((r, i) => (
                <div key={i} className="flex items-center justify-between gap-2 rounded-xl bg-violet/[0.03] px-3.5 py-2">
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-sm text-text-primary">{r.device_name}</p>
                    <p className="text-xs text-text-secondary">{r.customer_name}</p>
                  </div>
                  <span className="shrink-0 text-xs font-medium" style={{ color: statusColors[r.status] }}>{statusLabels[r.status]}</span>
                </div>
              ))
            )}
          </div>
