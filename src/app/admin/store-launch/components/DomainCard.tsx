...
export default function DomainCard({ category, tasks, expenses, allCategories }: DomainCardProps) {
  ...
  const [editingItem, setEditingItem] = useState<{ type: EditItemType, data: any } | null>(null);
  ...
  // Base color logic
  const baseColorClass = colorMap[category.color] || colorMap['slate'];
  const displayColorClass = isOverBudget ? colorMap['rose'] : baseColorClass;
  const dotColor = displayColorClass.split(' ')[0]; // just the bg-color
  ...
  return (
    <>
        ...
        <div className="h-full rounded-[calc(2rem-0.375rem)] bg-warm-surface/80 shadow-[inset_0_1px_1px_rgba(255,255,255,1)] dark:shadow-[inset_0_1px_1px_rgba(255,255,255,0.1)] p-6 md:p-8 flex flex-col gap-6 overflow-hidden">
          ...
          >
            ...
            <div className="flex items-center justify-between">
              ...
              <button 
                onClick={(e) => { e.stopPropagation(); setEditingItem({ type: "category", data: category }); }}
                className="p-1.5 rounded-full hover:bg-black/5 text-text-secondary opacity-0 group-hover:opacity-100 transition-opacity"
            ...
            <div className="flex items-center justify-between w-full">
              <div className="flex items-center gap-6 xl:gap-10 flex-1 pr-4">
                ...
                <div className="flex flex-col gap-1.5 w-full max-w-[160px]">
                  ...
                  <div className="h-1.5 w-full bg-warm-border/50 rounded-full overflow-hidden">
                    <div className={`h-full transition-all duration-1000 ${dotColor}`} style={{ width: `${burnRatePercent}%` }} />
                  </div>
                </div>
              </div>
                ...
                <div className="flex flex-col gap-2">
                  {tasks.map(task => (
                    <div key={task.id} className="flex items-center gap-3 p-3 rounded-xl bg-surface dark:bg-black/20 border border-warm-border/40 hover:border-violet/30 transition-colors group/task relative">
                      ...
                      <span className={`text-sm font-medium flex-1 pr-6 ${task.status === 'done' ? 'text-text-muted line-through' : 'text-text-primary'}`}>{task.title}</span>
                      
                      <button 
                        onClick={(e) => { e.stopPropagation(); setEditingItem({ type: "task", data: task }); }}
                        className="absolute right-3 opacity-0 group-hover/task:opacity-100 p-1 text-text-secondary hover:text-text-primary transition-opacity"
                        title="Редагувати"
                      >
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>
                      </button>
                  ...
                  {expenses.map(exp => (
                    <div key={exp.id} className="flex items-center justify-between p-3 rounded-xl bg-surface dark:bg-black/20 border border-warm-border/40 hover:border-violet/30 transition-colors group/exp relative">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button 
                          onClick={(e) => { e.stopPropagation(); setEditingItem({ type: "expense", data: exp }); }}
                          className="opacity-0 group-hover/exp:opacity-100 p-1 text-text-secondary hover:text-text-primary transition-opacity"
                          title="Редагувати"
                        >
                          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>
                        </button>
      ...
      <StoreLaunchEditModal 
        ...
        itemType={editingItem?.type || "task"} 
