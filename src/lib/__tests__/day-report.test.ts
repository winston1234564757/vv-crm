...
// Вартовий: якщо пояс не прибитий, решта тестів годин перевіряє випадковість.
describe("оточення тестів", () => {
  it("працює в тому самому поясі, що й рантайм — Europe/Kyiv", () => {
    expect(new Date("2026-07-25T12:00:00Z").getHours()).toBe(15);
  });
});
...
describe("dayNeighbours", () => {
  ...
  // Межа зліва включна: день ЗРАЗУ ПІСЛЯ епохи має бачити епоху як prev,
  // а не впиратись у неї на крок раніше (ловить `<` переплутане з `<=`).
  it("день одразу після епохи бачить епоху як prev", () => {
    expect(dayNeighbours("2026-07-22", "2026-07-21", "2026-07-30").prev).toBe("2026-07-21");
  });

  // Межа справа включна: день ЗРАЗУ ПЕРЕД сьогодні має бачити сьогодні як
  // next, а не впиратись у нього на крок раніше (ловить `>` переплутане з `>=`).
  it("день одразу перед сьогодні бачить сьогодні як next", () => {
    expect(dayNeighbours("2026-07-29", "2026-07-21", "2026-07-30").next).toBe("2026-07-30");
  });
...
describe("previousWorkingDay", () => {
  ...
  it("не бере сам день за базу, навіть якщо в нього є виторг", () => {
    const s = [
      { day: "2026-07-24", revenue: 500 },
      { day: "2026-07-25", revenue: 700 }, // цільовий день має виторг, але не база
    ];
    expect(previousWorkingDay("2026-07-25", s)).toBe("2026-07-24");
  });

  // Серія навмисно невідсортована: пізніший робочий день ("25") стоїть у
  // масиві ПІСЛЯ раннішого ("22"), щоб зловити наївну реалізацію, яка бере
  // перший підходящий по порядку масиву замість найближчого календарно.
  it("бере НАЙБЛИЖЧИЙ робочий день, а не перший по порядку масиву", () => {
    const s = [
      { day: "2026-07-25", revenue: 300 },
      { day: "2026-07-22", revenue: 900 },
    ];
    expect(previousWorkingDay("2026-07-27", s)).toBe("2026-07-25");
  });
});
