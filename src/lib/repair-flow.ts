 ...
 *
 * Ремонт заробляється видачею: робота здана клієнту — виторг наш. Якір —
 * `completed_at`, який `updateRepair`/`updateRepairStatus` ставлять рівно в
 * момент переходу в `handed_over`, і більше ніде.
 *
 * З цього правила є два наслідки, і обидва навмисні:
 *
 *  - Передоплата не є виторгом, поки пристрій не пішов. Гроші вже в касі й
 *    видно їх у «Гроші в наявності» — але заробленими вони стають на видачі.
 *  - Виданий у борг — виторг, попри те що каса порожня. Борг лишається
 *    дебіторкою: його окремо ловить блок уваги і показує рядок «у борг» у
 *    розбивці дня, щоб борг не виглядав готівкою.
 *
 * Це та сама дата, за якою ремонт стає рядком на сторінці Продажів
 * (`search_transactions`), — щоб одна операція не лежала на двох екранах у
 * різних днях.
 *
 * Ціна тут не при чому. Гарантійна переробка має ціну 0 за визначенням
 * (`updateRepair` обнуляє її примусово), безкоштовна діагностика — теж; без
 * них у P&L зникла б їхня собівартість і маржа поїхала б угору.
 */
...
export function repairSettledAt(r: SettleableRepair): string | null {
  ...
  if (delivered && r.completed_at) return r.completed_at;
