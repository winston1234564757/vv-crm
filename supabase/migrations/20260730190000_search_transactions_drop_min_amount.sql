-- Відкат порогу суми (20260730170000). Він додавався під розіграш «участь від
-- 300 ₴»; розіграшу не буде, а від тієї роботи потрібне було лише сортування
-- за сумою (20260730160000) — воно лишається.
--
-- DROP, а не CREATE OR REPLACE: прибрати параметр заміною не можна, вийшло б
-- два перевантаження з дефолтами на всі аргументи, і будь-який виклик упав би
-- на неоднозначності. Права після DROP не переживають, тому GRANT нижче.
--
-- Тіло — точна копія 20260730170000 без обох рядків `p_min_amount`.

drop function if exists public.search_transactions(
