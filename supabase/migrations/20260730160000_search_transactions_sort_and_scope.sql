-- Сортування списку операцій за сумою + обмеження вибірки фінансовою епохою.
--
-- Обидва рішення мусять жити тут, а не в React: пагінація серверна, тож
-- сортування на клієнті впорядкувало б лише поточні 25 рядків. Для розіграшу
-- «найбільший чек» це не деталь, а різниця між правильним і випадковим
-- переможцем.
--
-- Нове:
--   p_from — нижня межа по даті операції. Сторінка передає туди
--            `finance_epoch` із settings, тож продажі «з рук» до відкриття
--            магазину в топ не лізуть. NULL = увесь час, як було.
--   p_sort — 'date' (як було) або 'amount'.
--   p_dir  — 'desc' (як було) або 'asc'.
--   amount — сума операції в результаті: `sales.total_amount` для продажу,
--            `repairs.price` для ремонту. Однаково потрібна для ORDER BY, тож
--            дешевше віддати її нагору, ніж рахувати вдруге в застосунку.
--
-- Порядок завжди добивається `row_id` останнім ключем. Без цього два чеки з
-- однаковою сумою (у магазині вже є дві по 3700 ₴) могли роз'їхатися між
-- сторінками — Postgres не зобов'язаний тримати стабільний порядок для рівних
-- ключів, і той самий рядок з'явився б двічі або зник.
--
...
drop function if exists public.search_transactions(text, text, text, integer, integer);
...
grant execute on function public.search_transactions(
  text, text, text, integer, integer, timestamptz, text, text
